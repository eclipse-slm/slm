...
Deployment Capabilities are used to deploy a service on a resource. They can be added dynamically via the REST API of the [Resource Registry](/docs/usage/common/api#resource-registry).
...
::: warning ATTENTION
`<<your-slm-host>>` and [`<<your-keycloak-token>>`](/docs/usage/common/api#authentication) must be replaced!
:::
```sh
curl -X 'POST' \
