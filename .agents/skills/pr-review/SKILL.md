---
name: pr-review
description: Reviews pull requests and code changes in an Ansible collection against
  project standards and the Ansible Collection Review Checklist. Use when asked to
  review a PR, patch, diff, or set of code changes. Do not use for GitHub Issues or
  general Q&A.
---
# Skill: pr-review
...
Review pull requests and code changes in an Ansible collection.
...
- Module file names follow the collection's established naming pattern.
...
- No sensitive data appears in return values in plaintext.

### Error Handling

- All errors call `module.fail_json(msg=...)` with a descriptive, actionable message — no bare `raise` or `sys.exit()`.
- Read-only modules handle privilege errors gracefully (return partial results, not a failure).
- Shared utilities from `module_utils/` are used for common operations.
...
- Integration test pattern is followed:
  ...
  3. Verify actual state via a separate module or command -> `register: result` -> `ansible.builtin.assert`
- Each integration test target has `tests/integration/targets/<name>/meta/main.yml` declaring dependencies.
