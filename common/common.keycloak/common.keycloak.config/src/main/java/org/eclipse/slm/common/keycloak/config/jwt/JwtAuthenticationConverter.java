...
public class JwtAuthenticationConverter implements Converter<Jwt, JwtAuthenticationToken> {
    ...
    public JwtAuthenticationToken convert(Jwt jwt) {
        ...
        final var authoritiesPreparedForSpring = new ArrayList<GrantedAuthority>();
        for (var authority : authorities) {
            var convertedAuthority = new SimpleGrantedAuthority("ROLE_" + authority.getAuthority());
            authoritiesPreparedForSpring.add(convertedAuthority);
        }
        final String username = JsonPath.read(jwt.getClaims(), issuerProperties.getUsernameJsonPath());
        return new JwtAuthenticationToken(jwt, authoritiesPreparedForSpring, username);
    }
}
