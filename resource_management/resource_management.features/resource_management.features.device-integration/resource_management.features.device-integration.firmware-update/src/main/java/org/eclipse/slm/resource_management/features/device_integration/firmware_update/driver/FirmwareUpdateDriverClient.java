...
public class FirmwareUpdateDriverClient extends AbstractDriverClient {
    ...
    public void prepareFirmwareUpdate(UUID firmwareUpdateJobId,
                                      String connectionParametersBade64Encoded,
                                      byte[] firmwareUpdateFile,
                                      CredentialData credentialData,
                                      FirmwareUpdateJobListener firmwareUpdateJobListener) {
        ...
        ArtefactUpdate.ArtefactChunk chunkMetaData = this.buildMetadataArtefactChunk(firmwareUpdateJobId.toString(), connectionParametersBade64Encoded, credentialData);
        prepareRequestObserver.onNext(chunkMetaData);
    }

    public void activateFirmwareUpdate(UUID firmwareUpdateJobId,
                                       String connectionParametersBade64Encoded,
                                       byte[] firmwareUpdateFile,
                                       CredentialData credentialData,
                                       FirmwareUpdateJobListener firmwareUpdateJobListener) {
    ...
    private ArtefactUpdate.ArtefactChunk buildMetadataArtefactChunk(String firmwareUpdateJobId, String connectionParametersBade64Encoded, CredentialData credentialData) {
        var deviceIdentifierBlob = ByteString.copyFromUtf8(connectionParametersBade64Encoded);

        var metadataBuilder = ArtefactUpdate.ArtefactMetaData.newBuilder()
        ...
        if (credentialData != null) {
            if (!(credentialData instanceof CredentialDataUsernamePassword usernamePasswordCredentialData)) {
                throw new IllegalArgumentException("Currently only 'CredentialDataUsernamePassword' is supported for firmware update, not " + credentialData.getClass().getName());
            }

            metadataBuilder.setDeviceCredentials(ArtefactUpdate.DeviceCredentials.newBuilder()
                    .setUsername(usernamePasswordCredentialData.getUsername())
                    .setPassword(usernamePasswordCredentialData.getPassword())
            );
        }
